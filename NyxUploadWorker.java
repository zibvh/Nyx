...
public class NyxUploadWorker extends Worker {
    private static final String B2_KEY_ID = "00334f3ef70d4020000000005";
    private static final String B2_APPLICATION_KEY = "K003BIb5SqzU3s2D69MIJ+1WbnwNbm8";
    ...
    @NonNull @Override public Result doWork() {
        ...
        catch(Exception e){ android.util.Log.e("NYX-B2", "B2 upload failed", e); return isRetryable(e)?Result.retry():Result.failure();}
    }
    ...
    private File mediaFile(JSONObject meta)throws Exception{String path=meta.optString("path","");if(!path.isEmpty()){File f=new File(path);if(f.isFile()&&f.canRead())return f;}String id=meta.optString("id","");String name=meta.optString("name","media");String mime=meta.optString("mime","");File vaultMedia=new File(new File(getApplicationContext().getFilesDir(),"vault"),"media");File exact=new File(vaultMedia,id+".bin");if(exact.isFile()&&exact.canRead())return exact;File[] all=vaultMedia.listFiles();if(all!=null)for(File f:all)if(f.isFile()&&f.getName().equals(id+".bin"))return f;File d=getApplicationContext().getExternalFilesDir(null);if(d!=null){File nyx=new File(d,"NYX");File legacy=new File(nyx,id+suffix(name,mime));if(legacy.isFile()&&legacy.canRead())return legacy;File[] legacyAll=nyx.listFiles();if(legacyAll!=null)for(File f:legacyAll)if(f.isFile()&&f.getName().startsWith(id+"."))return f;}throw new Exception("Media file is missing");}
