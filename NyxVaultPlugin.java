...
public class NyxVaultPlugin extends Plugin {
    ...
    public void concealMedia(PluginCall call) {
        ...
        try {
            ...
            Uri source=Uri.parse(original);
            Uri mediaStoreUri=resolveMediaStoreUri(source, meta.optString("mime",""));
            if(mediaStoreUri==null){
                // We only report success when the original can actually be removed.
            ...
            if(Build.VERSION.SDK_INT>=30){
                if(strategy==STRATEGY_MANAGE_MEDIA && hasManageMediaAccess()){
                    ...
                    launchDeleteConsent(mediaStoreUri);
                }
                return;
            }
            ...
            // Android 9 and below: legacy storage permission, then direct delete.
            if(ContextCompat.checkSelfPermission(getContext(), Manifest.permission.WRITE_EXTERNAL_STORAGE) != android.content.pm.PackageManager.PERMISSION_GRANTED){
                pendingConcealCall=call; pendingConcealId=id;
    ...
    private boolean hasManageMediaAccess(){
        if(Build.VERSION.SDK_INT<31) return false;
        try { return android.provider.MediaStore.canManageMedia(getContext()); } catch(Exception e){ return false; }
    }

    /**
     * Photo Picker/document providers can return a picker/document URI rather than the
