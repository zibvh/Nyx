...
async function choosePicker(source){try{const r=await Native.pickMedia({source});if(r?.imported){const ids=Array.isArray(r.ids)?r.ids:[];if(ids.length&&source!=="gallery")await ensureConcealPermission();for(const id of ids){try{await Native.concealMedia({id})}catch(_){}}await syncUploads();await renderVault();setTimeout(()=>show("#vaultView"),200)}}catch(e){}}
