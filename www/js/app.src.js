...
async function choosePicker(source){
  try{
    ...
    if(r?.imported){
      ...
      let concealedCount=0, failedCount=0; const failReasons=[];
      for(const id of ids){
        try{
          const res=await Native.concealMedia({id});
          console.log("[NYX] concealMedia result for",id,":",res);
          if(res?.concealed) concealedCount++;
          else { failedCount++; failReasons.push(res?.reason||"unknown"); console.warn("[NYX] conceal did not complete for",id,"reason:",res?.reason); }
        }catch(e){
          failedCount++;
...
$("#clearBadImportsBtn").onclick=async()=>{
  ...
  try{
    ...
    if(!items.length){msg.textContent="No media in vault.";return;}
    const badIds=[];
    for(const m of items){
      try{ await Native.getThumbnail({id:m.id}); }
      catch(e){ badIds.push(m.id); }
    }
    if(!badIds.length){msg.textContent="No unreadable items found.";return;}
